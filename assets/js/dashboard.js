...
async function renderAdminDashboardChart() {
    const ctx = document.getElementById('reservationsChart');
    if (!ctx) return;

    try {
        const response = await fetch(`${MOCKAPI_BASE_URL}/reservations`);
        ...
        console.error("Error al renderizar el dashboard de Admin:", error);
        const container = document.getElementById('adminDashboardChart');
        if (container) container.innerHTML = '<div class="alert alert-danger mt-3">No se pudo cargar el gráfico de estado de reservas.</div>';
    }
}
